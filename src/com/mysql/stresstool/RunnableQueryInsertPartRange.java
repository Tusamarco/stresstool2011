 ...
 * @version 1.0
 */
public  class RunnableQueryInsertPartRange extends RunnableInsertBasic implements Runnable, RunnableQueryInsertInterface {
	...
	private static final ArrayList <String> CLASS_PARAMETERS = new ArrayList(Arrays.asList(
			...
			"yearstart","monthstart","daystart","daystotal","partitionType","debug","stikyconnection"));
		
	public RunnableQueryInsertPartRange() {
		try {
	...
	public void run() {
		...
		try {
			if(stikyconnection && jdbcUrlMap.get("dbType") != null &&  !((String)jdbcUrlMap.get("dbType")).equals("MySQL"))
			{
