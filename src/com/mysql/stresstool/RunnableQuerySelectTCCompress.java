 ...
 * @version 1.0
 */
public  class RunnableQuerySelectTCCompress  extends RunnableQuerySelectBasic  {
    ...
    String generateSelectString(int pkStart, int pkEnds, String select) {
		...
		if(getDbType().equals("MySQL") && !this.isDoSimplePk() && getNumberOfJoinTables() == 0){
			select = "Select "
						+ primaryTable + ".a, UNCOMPRESSED_LENGTH(b),uncompress(b), UNCOMPRESSED_LENGTH(c),uncompress(c),counter,"
						+ primaryTable +".time,"
	    ...
	    else if(getDbType().equals("MySQL") && !this.isDoSimplePk() && getNumberOfJoinTables() > 0)
		{
			...
			select = "Select "
								+ primaryTable +".a,uncompress(b),uncompress(c),counter,"
								+ primaryTable +".time,"
								+ primaryTable +".partitionid,"
								+ primaryTable +".strrecordtype, "
								+ "UNCOMPRESSED_LENGTH(" + secondaryChildRetieved +".stroperation),UNCOMPRESS(" + secondaryChildRetieved +".stroperation)" 
								+ childFieldsToShow 
		...
		else if(this.isDoSimplePk())
		{
		    ...
		    select =
		        "Select "+ primaryTable +".a,b,c,counter,"+ primaryTable +".time,uncompress(strrecordtype) ";
		    select = select +
