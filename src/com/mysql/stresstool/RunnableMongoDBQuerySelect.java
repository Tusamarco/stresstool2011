 ...
 * @version 1.0
 */
public  class RunnableMongoDBQuerySelect  implements Runnable,RunnableSelectQueryInterface {
...
//    Connection conn = null;
    ...
    public RunnableMongoDBQuerySelect() {
        try {
...
//                this.setExecutionTime(executionTime);
	 ...
	 * @return
	 */
	private String generateSelectString(int pkStart, int pkEnds) {
		String select = null;
		...
		if(this.forceIndex && !this.getIndexName().equals("")){
		...
		if(dbType.equals("MySQL") && !this.isDoSimplePk() && getNumberOfJoinTables() == 0){
			select = "Select "+ primaryTable +".a,b,c,counter,"+ primaryTable +".time,"+ primaryTable +".partitionid,"+ primaryTable +".strrecordtype" ;
			
			if(this.getSelectFilterMethod().toLowerCase().equals("range")){
				select = select + "from "+ primaryTable + indexHint +" where "+ primaryTable +".a > " + pkStart + " and "+ primaryTable +".a <" + pkEnds + " limit " + this.getSelectLimit();
			}
	    ...
	    else if(dbType.equals("MySQL") && !this.isDoSimplePk() && getNumberOfJoinTables() > 0)
		{
			...
			select = "Select "+ primaryTable +".a,b,c,counter,"+ primaryTable +".time,"+ primaryTable +".partitionid,"+ primaryTable +".strrecordtype, "+ secondaryChildRetieved +".stroperation" + childFieldsToShow + childJoinsConditions;
			
			if(this.getSelectFilterMethod().toLowerCase().equals("range")){
				select = select + " where "+ primaryTable +".a > " + pkStart + " and "+ primaryTable +".a <" + pkEnds + " limit "+ this.getSelectLimit();
			}
			else if (this.getSelectFilterMethod().toLowerCase().equals("in")){
				String sPk ="";
				sPk = calculaltePK(pkStart, pkEnds, sPk);
...
//		System.out.println(select);
	...
	private PrimaryKeyRangeDefiner setPkRange(Connection conn,PrimaryKeyRangeDefiner pkRange) {
		...
		if(pkRange.getLen() == 0){
			...
			{
				int min = StressTool.getNumberFromRandomMinMax(values[0],values[1]).intValue();
				pkRange.setKeyStart(min);
				pkRange.setKeyEnd(StressTool.getNumberFromRandomMinMaxCeling(min, values[1],getNumberOfIntervalKeys()).intValue());
			}
		}
	...
	private String calculaltePK(int pkStart, int pkEnds, String sPk) {
		int iloop =0;
		int startValue = StressTool.getNumberFromRandom(pkEnds).intValue();
		for(int iPk = startValue; iPk <= pkEnds; iPk++ )
		{
			sPk = sPk + iPk;
